...
fn repeat(value: &PyAny, n_times: PyExpr) -> PyExpr {
    if let Ok(true) = value.is_instance_of::<PyBool>() {
        ...
        polars_rs::lazy::dsl::repeat(val, n_times.inner).into()
    } else if let Ok(int) = value.downcast::<PyInt>() {
        let val = int.extract::<i64>().unwrap();

        if val > 0 && val < i32::MAX as i64 || val < 0 && val > i32::MIN as i64 {
            polars_rs::lazy::dsl::repeat(val as i32, n_times.inner).into()
        } else {
            polars_rs::lazy::dsl::repeat(val, n_times.inner).into()
        }
        ...
        polars_rs::lazy::dsl::repeat(Null {}, n_times.inner).into()
    } else {
...
fn pearson_corr(a: dsl::PyExpr, b: dsl::PyExpr) -> dsl::PyExpr {
    polars_rs::lazy::dsl::pearson_corr(a.inner, b.inner).into()
}

#[pyfunction]
fn spearman_rank_corr(a: dsl::PyExpr, b: dsl::PyExpr) -> dsl::PyExpr {
    polars_rs::lazy::dsl::spearman_rank_corr(a.inner, b.inner).into()
}
...
fn py_date_range(
    ...
    polars_rs::time::date_range_impl(name, start, stop, Duration::parse(every), closed.0, tu)
        .into_series()
        .into()
}
